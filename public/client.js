// Give some nice graphics to the chat
...
// Slim Scroll librairy which implement a nice scroll bar for the chat messages
(function(d){jQuery.fn.extend({slimScroll:function(o){var a=ops=d.extend({wheelStep:20,width:"auto",height:"250px",size:"7px",color:"#000",position:"right",distance:"-9px",start:"top",opacity:0.4,alwaysVisible:!1,railVisible:!1,railColor:"#333",railOpacity:"0.2",railClass:"slimScrollRail",barClass:"slimScrollBar",wrapperClass:"slimScrollDiv",allowPageScroll:!1,scroll:0},o);this.each(function(){function h(a,d,e){var f=a;d&&(f=parseInt(c.css("top"))+a*B/100*c.outerHeight(),d=b.outerHeight()-c.outerHeight(),
...
$(function() {
	// Get the value of the message input and the submit button
	messageContainer = $('#messageInput');
	submitButton = $("#submit");
	bindButton();
	window.setInterval(time, 1000*10);
	$("#alertPseudo").hide();
	$('#Identifiaction').modal('show');
	$("#pseudoSubmit").click(function() {setPseudo()});
	$("#messages").slimScroll({height: '600px'});
...
//Function realitve to Socket.io
var socket = io.connect();

function sentMessage() {
	// We check if the message is not empy
	if (messageContainer.val() != "") 
	{
		// If the user still not have pseudo we display the Username page
		if (pseudo == "") 
		{
			$('#Identifiaction').modal('show');
		}
		else 
		{
			...
			addMessage(messageContainer.val(), "Me", nbMessage,userColor, new Date().toISOString(), true);
			nbMessage++;
			// Clear out the message text area
...
function addMessage(msg, pseudo, id, colors, date, self) {

	/* Depends on a self message or not, the view won't be the same*/
	if(self) var classDiv = "row message me";
	else var classDiv = "row message";
	
	/* Add the message to the view */
	$("#messages").append('<div class="'+classDiv+'" id="'+id+'"><p class="informations"> </span> <span class="pseudo">  &nbsp; '+pseudo+'</span>, <time class="date" title="'+date+'">'+date+'</time> <span class="icon-chevron-left"></span><span class="icon-chevron-right">  </p><p class="textemessage">' + msg + '</p></div>');
	
	// Change the border color to identify each user
	$("#"+id).css("border-right-color", '#'+colors);
	$("#"+id).css("border-left-color", '#'+colors);

	$("#"+id).find(".icon-chevron-right").click(function() {
		displayOldChange(id,1);
	});
	
	$("#"+id).find(".icon-chevron-left").click(function() {
		displayOldChange(id,0);
	});
	time();
	$("#"+id).find(".textemessage").mousedown(function() {clickChangeMessage(id,userColor)});
}

function clickChangeMessage(id,colors) {
    var divTxt = $("#"+id).find(".textemessage").text();
	var divHtml = $("#"+id).find(".textemessage").html();
	
    var editableText = $("<textarea id="+id+"/>");
    editableText.val(divTxt);
    $("#"+id).find(".textemessage").replaceWith(editableText);
	$("#"+id).mouseup(function() { editableText.focus()});
    // setup the blur event for this new textarea
    editableText.blur(function() {validateChangeMessage(id,colors,divHtml,divTxt)});
}

function validateChangeMessage(id,colors,oldHtml,oldVal) {

    var val = $("#"+id).find("textarea").val();
    var viewableText = $('<p class="textemessage" />');
	// If we change nothing we let the old html with the former difference
	if (oldVal == val) {
		viewableText.html(oldHtml);
		 $("#"+id).find("textarea").replaceWith(viewableText);
	}
	else {
		viewableText.html(val);
		 $("#"+id).find("textarea").replaceWith(viewableText);
		 // Send the message to the server using socket.io
		var transmit = {message: val, id: id, color: colors};
		socket.emit('changeMessage', transmit);
	}
			
	// setup the click event for this new div
    $("#"+id).find(".textemessage").mousedown(function() {clickChangeMessage(id,colors)});
	
}

function autochangeMessage(id,msg) {
	$("#"+id).find(".textemessage").html(msg);
}

function displayOldChange(id,sens) {
	var transmit = {id: id, sens: sens}; 
	socket.emit('displayOldChange', transmit);
	
}

// Slim scroll configuration
function setHeight() {
	$(".slimScrollDiv").height('603');
	$(".slimScrollDiv").css('overflow', 'visible')
}

// The submit button will be inactive if there is no text typed.
function bindButton() {
	submitButton.button('loading');
...
function setPseudo() {
	if ($("#pseudoInput").val() != "")
	{
		...
		socket.on('pseudoAttribute', function(data){
			nbMessage = data['nbMessage'] ;
			// Check if the pseudo hasn't already been picked
			if(data['statu'] == "ok")
			{
				$('#Identifiaction').modal('hide');
				$("#alertPseudo").hide();
...
// Display the time the message has been added
function time() {
	$("time").each(function(){
...
// Nb User to the server
socket.on('nbUsers', function(msg) {
	$("#nbUsers").html(msg.nb);
});
// New Message from other users
socket.on('message', function(data) {
	addMessage(data['message'], data['pseudo'], data['id'], data['color'], new Date().toISOString(), false);
	nbMessage++;
	console.log(data);
});
// Changing message from other users
socket.on('changeMessage', function(data) {
	autochangeMessage(data['id'],data['message']);
});